...
if __name__ == "__main__":
    ...
    for incident in security_incidents:
        if incident.get("Attacker"):
            attackers.update(incident["Attacker"])
        ...
        for intrusion_set_data in intrusion_sets_data:
            ...
            for line in intrusion_set_output.splitlines():
                ...
                if line.startswith("入侵集合名稱:"):
                    ...
                    intrusion_set_json["match_type"] = "name" if intrusion_set_json[
                                                                     "match_type"] == "名稱匹配" else "aliases"
            ...
            if malware_ids:
                for m_id in malware_ids[:]:
                    ...
                    current_relationship = None
                    for line in malware_output.splitlines():
                        line = line.rstrip()
